...
func (p *PPU) DumpNametables() []image.Image {
	...
	nametables := make([][960]uint8, 4)

	var pixelMap = [4]color.RGBA{
		{R: 0x00, G: 0x00, B: 0x00, A: 0xFF},
		{R: 0x55, G: 0x55, B: 0xFF, A: 0xFF},
		{R: 0xFF, G: 0x55, B: 0x55, A: 0xFF},
		{R: 0xFF, G: 0xFF, B: 0xAA, A: 0xFF},
	}

	for i, offset := range nametablesOffsets {
		for j := range nametableSize {
			nametables[i][j] = p.bus.Read(offset + j)
		}
	}
	...
	for _, nametable := range nametables {
		...
		for _, tileIndex := range nametable {
			...
			for i := range uint16(8) {
				loBits := p.bus.Read(0x1000 + uint16(tileIndex)*16 + i)
				hiBits := p.bus.Read(0x1000 + uint16(tileIndex)*16 + i + 8)
