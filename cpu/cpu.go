...
func (c *CPU) Push(value uint8) {
	...
	c.Bus.Write(stackAddress, value)
}

func (c *CPU) Pop() uint8 {
	stackAddress := uint16(c.Sp) | 0x0100
	value := c.Bus.Read(stackAddress)
	c.Sp++
	return value
}
...
func (c *CPU) fetchNextValue(addressingMode AddressingMode) uint16 {
	...
	case XIndexedAbsoluteValue:
		...
		hi := c.Bus.Read(c.Pc)
		c.Pc++
		addr := uint16(hi)<<8 + uint16(lo) + uint16(c.X)
	...
	case AbsoluteIndirect:
		...
		lo = c.Bus.Read(indirectAddr)
		hi = c.Bus.Read(indirectAddr + 1)
	...
	case ZeroPageIndirectYIndexedValue:
		addr := uint16(c.Bus.Read(c.Pc)) + uint16(c.Y)
		c.Pc++
		lo := c.Bus.Read(addr)
		hi := c.Bus.Read(addr + 1)
		addr = uint16(hi)<<8 + uint16(lo)
